...
public class ConsultationController {
    ...
    public List<TestRequest> getForConsultations()  {
        ...
        try{
            return testRequestQueryService.findBy(RequestStatus.LAB_TEST_COMPLETED); // fetching all test requests for which lab test is marked completed
    ...
    public List<TestRequest> getForDoctor()  {
        ...
        // For reference check the method getForTests() method from LabRequestController class
        try {
            User loggedInDoctor = userLoggedInService.getLoggedInUser(); // fetching details of currently logged-in doctor
            return testRequestQueryService.findByDoctor(loggedInDoctor); // fetching all test requests assigned to logged-in doctor
        }catch (AppException e) {
            throw asBadRequest(e.getMessage());
        }
    ...
    public TestRequest assignForConsultation(@PathVariable Long id) {
        ...
        try {
            User loggedInDoctor = userLoggedInService.getLoggedInUser(); // fetching details of currently logged-in doctor
            return testRequestUpdateService.assignForConsultation(id,loggedInDoctor); // assigning the specific test request(by id) to the logged-in doctor for consultation
        }catch (AppException e) {
            throw asBadRequest(e.getMessage());
        }
    }
    ...
    public TestRequest updateConsultation(@PathVariable Long id,@RequestBody CreateConsultationRequest testResult) {
        ...
        try {
            User loggedInDoctor = userLoggedInService.getLoggedInUser(); // fetching details of currently logged-in doctor
            return testRequestUpdateService.updateConsultation(id,testResult,loggedInDoctor);// updating the consultation remarks for selected test request(by id) by the  doctor
